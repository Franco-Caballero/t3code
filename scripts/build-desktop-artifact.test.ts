...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("keeps the custom desktop product identity on every upstream channel", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code Scroll");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Code Scroll");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.equal((win.win as Record<string, unknown>).executableName, "t3code-scroll");
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Code Scroll 1.2.3 Installer",
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.t3tools.t3code.scroll",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.t3tools.t3code.scroll</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.t3tools.t3code.scroll");
